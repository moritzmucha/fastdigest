...
impl PyTDigest {
    ...
    /// Compresses the digest to have at most `max_centroids` centroids (in‑place).
    ...
    pub fn trimmed_mean(&self, q1: f64, q2: f64) -> PyResult<f64> {
        ...
        for centroid in centroids {
            ...
            let overlap = (
                c_end.min(upper_weight_threshold)
    ...
    /// each with keys "m" (mean) and "c" (weight).
    ///
    /// This structure is chosen to be compatible with the "tdigest" Python library.
    ...
    pub fn from_dict<'py>(tdigest_dict: &Bound<'py, PyDict>) -> PyResult<Self> {
        let centroids_obj = tdigest_dict.get_item("centroids")?
            .ok_or_else(|| {
        ...
        for item in centroids_list.iter() {
            ...
            let mean: f64 = d.get_item("m")?
                .ok_or_else(|| PyKeyError::new_err("Centroid missing 'm' key"))?
                .extract()?;
            let weight: f64 = d.get_item("c")?
                .ok_or_else(|| PyKeyError::new_err("Centroid missing 'c' key"))?
                .extract()?;
            centroids.push(Centroid::new(mean, weight));
        }
        if centroids.is_empty() {
            return Err(PyValueError::new_err("Centroids list cannot be empty"));
        }
        Ok(Self {
